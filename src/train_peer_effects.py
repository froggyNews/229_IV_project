"""Simplified peer effects training utilities."""
...
@dataclass
class PeerConfig:
    """Configuration for peer effects analysis for a single target."""
...
def prepare_peer_dataset(cfg: PeerConfig, cores: Dict[str, pd.DataFrame], debug: bool = False) -> pd.DataFrame:
    ...
    if target_col in dataset.columns:
        ...
        if len(dataset) == 0:
            ...
            print(f"  - Total values: {initial_rows}")
            return pd.DataFrame()
    ...
    if cfg.include_self_lag and len(dataset) > 0:
        ...
        if "ticker" in dataset.columns:
            ...
            dataset["y_lag1"] = dataset[target_col].shift(1)
...
def train_peer_model(dataset: pd.DataFrame, test_frac: float) -> Tuple[xgb.XGBRegressor, Dict[str, Any], List[str]]:
    """Train a model to predict the target using peer features."""
...
def analyze_peer_effects(model: xgb.XGBRegressor, feature_names: Sequence[str]) -> Dict[str, Any]:
    ...
    feature_imp = pd.DataFrame({"feature": names, "importance": importances}).sort_values(
        "importance", ascending=False
    )

    peer_effects: Dict[str, float] = {}
...
def run_peer_analysis(cfg: PeerConfig, cores: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"target": cfg.target, "status": "error", "error": str(e)}
    ...
    for target in targets:
        ...
        try:
            ...
            cfg = PeerConfig(
                ...
                tolerance=tolerance,
                db_path=db_path,
                include_self_lag=include_self_lag,
                exclude_contemporaneous=exclude_contemporaneous,
            ...
            if dataset.empty:
                ...
                results[target] = {
                    ...
                    "error": "Dataset is empty after preparation",
                }
                continue
...
if __name__ == "__main__":  # pragma: no cover - manual testing
    ...
    cores = {t: load_ticker_core(t, start="2025-01-01", end="2025-01-02") for t in tickers}
